# Relatório de Validação
## Apresentação via Google Meet
### Pautas Levantadas
1. Introdução do ODS - 11 Cidades e Comunidades Sustentáveis
3. Objetivo da Aplicação
2. Requisitos Funcionais 
4. Processo de Desenvolvimento
5. Demonstração do Layout e das Funcionalidades Aplicadas
6. Perguntas e Respostas

### 1. Introdução do ODS - 11 Cidades e Comunidades Sustentáveis
- Missão: “Tornar as cidades e comunidades mais inclusivas, seguras, resilientes e sustentáveis”

### 2. Objetivo da Aplicação
- Cenário atual do lixo na cidade de Fortaleza
- Como foi pensado
- Como a aplicação pode ser utilizada no dia-a-dia

### 3. Requisitos Funcionais
- Pesquisa por pontos de coleta de lixo na cidade de Fortaleza
- Cadastro de novos ecopontos
- Endereços localizados no mapa

### 4. Processo de Desenvolvimento
- Ferramentas utilizadas
- Tecnologias ulizadas 
- Reuniões da equipe e conclusões alcançadas
- Divisão de tarefas da equipe

### 5. Demonstração do Layout e das Funcionalidades Aplicadas
- Demonstração visual do funcionamento da aplicação, com exemplos do uso de cada funcionalidade

### 6. Perguntas e Respostas
- Momento tira-dúvidas do público-alvo sobre a  apresentação
